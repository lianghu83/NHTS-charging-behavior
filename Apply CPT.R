...
f <- function(filename) {
  ...
  start_range <- runif(1, min = 0.8*df$RANGE[1], max = df$RANGE[1])
  for (i in 1:nrow(df)) {
    ...
    df$Decision[i] <- ifelse(df$Charger[i]==1 & df$DWELTIME_new[i]>time_hassel, 
                             CPTChargingDecision(df$SOC_D[i], df$Between_d[i], df$DWELTIME_new[i], df$Power[i], df$DESTINATION[i], df$RANGE[1]), 
                             0)
