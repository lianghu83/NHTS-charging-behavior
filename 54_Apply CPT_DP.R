...
set.seed(2017)
filename <- sample(filename, size=bev_num, replace=FALSE) #size=bev_num
#select BEV
...
f <- function(filename) {
  ...
  #source ArriveTimeToNextCharger.R to calculate the arrival time at next charges
  #source("C:\\Users\\lianghu\\Desktop\\Smart mobility charging behavior project\\code\\ArriveTimeToNextCharger.R")
  #df$ARRIVETIME_next <- ArriveTimeToNextCharger(df[, c('Charger', 'ENDTIME_new')])
  
  #source charging decision function CPTChargingDecision_6_DP.R
  source("C:\\Users\\lianghu\\Desktop\\Smart mobility charging behavior project\\code\\CPTChargingDecision_6_DP.R")
  ...
  for (i in 1:nrow(df)) {
    ...
    #                         CPTChargingDecision(df$SOC_D[i], df$Between_d[i], df$DWELTIME_new[i], df$Power[i], df$DESTINATION[i], df$RANGE[1], df$ENDTIME[i], df$WHYTO[i]), 
    #                         0)
    #option C. merge option A and B
    df$Decision[i] <- ifelse(df$Charger[i]==1 & df$DWELTIME_new[i]>time_hassel, 
                             CPTChargingDecisionLookAhead(df$SOC_D[i], df$Between_d[i], df$ToLast_d[i], df$DWELTIME_new[i], df$Power[i], df$DESTINATION[i], df$RANGE[1], df$ENDTIME[i], df$WHYTO[i], df$ToLast_charger[i]), 
                             0)
    ...
    #home charging, back to home
    if (i==nrow(df) & df$DESTINATION[i]=='home' & df$DWELTIME_new[i]>time_hassel) {
      #check whether delay charging is Yes?
      if (df$DWELTIME_new[i]/60*df$Power[i]/consumption > (df$RANGE[1]-df$SOC_D[i])) {
        charging_duration <- round((df$RANGE[1]-df$SOC_D[i])*consumption/df$Power[i]*60, 0) #minutes
        empty_duration <- df$DWELTIME_new[i] - charging_duration #minutes
        df$STRTTIME_charging[i] <- WhenStartDelayedCharging(df$ENDTIME[i], empty_duration, charging_duration)
        #df$Decision[i] <- CPTChargingDecision(df$SOC_D[i], df$Between_d[i], df$DWELTIME_new[i], df$Power[i], df$DESTINATION[i], df$RANGE[1], df$STRTTIME_charging[i], df$WHYTO[i])
        df$Decision[i] <- CPTChargingDecisionLookAhead(df$SOC_D[i], df$Between_d[i], df$ToLast_d[i], df$DWELTIME_new[i], df$Power[i], df$DESTINATION[i], df$RANGE[1], df$STRTTIME_charging[i], df$WHYTO[i], df$ToLast_charger[i])
        df$SOC_add[i] <- (df$RANGE[1]-df$SOC_D[i]) * df$Decision[i]
      }
    }
  }
