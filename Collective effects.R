...
GridPower <- function(aa, place) {
  ...
  #aa$charging_start <- aa$ENDTIME%/%100*60+aa$ENDTIME%%100 #standard: ENDTIME
  aa$charging_start <- aa$STRTTIME_charging%/%100*60+aa$STRTTIME_charging%%100 #delay charging: STRTTIME_charging
...
#summary grid_total
mean(grid_total$power3)
sum(grid_total$power3)
(grid_total$power3-mean(grid_total$power3))/mean(grid_total$power3)
...
ChargingProfileStackBar <- function(grid) {
  ...
  p <- ggplot(grid_sub, aes(x=hour, y=power3/1000/1000, fill=place)) +
    ...
    theme(axis.title=element_text(size=9), 
          ...
          panel.grid.minor=element_blank(),
          #legend
          legend.position=c(0.1,0.85), #raw: (0.1,0.85)
          legend.margin=margin(0,0,0,0),
          #legend.direction="horizontal",
          legend.key.size=unit(0.15,'in'),
          legend.text=element_text(size=8),
...
#a=b
ct_name <- paste0('charge_table_S2_R2_ab=', as.character(seq(5,95,10)/100), '.csv')
...
ImpactOnSOCAlphaBeta <- function(ct) {
  ...
  p <- ggplot(ct_sub, aes(x=alpha_beta)) +
    ...
    scale_y_continuous(limits=c(33,42), breaks=seq(33,42,1)) + #limits=c(20,55)
    theme_bw() +
    theme(axis.title=element_text(size=9), 
          axis.text = element_text(size=8),
          panel.grid.major=element_blank(),
...
p
#ggsave(paste0(image_path, "alpha=beta_R2.jpeg"), p, width=3.2, height=2, units='in')



#lambda
ct_name <- paste0('charge_table_S2_R2_l=', as.character(seq(1.25,5.75,0.5)), '.csv')
setwd("H:\\NHTS2017\\Result Data")
ct <- do.call("rbind", lapply(ct_name, read.csv))
...
ImpactOnSOCLambda <- function(ct) {
  ...
  p <- ggplot(ct_sub, aes(x=lambda)) +
    ...
    scale_y_continuous(limits=c(33,42), breaks=seq(33,42,1)) +
    theme_bw() +
    theme(axis.title=element_text(size=9), 
          axis.text = element_text(size=8),
          panel.grid.major=element_blank(),
...
p
#ggsave(paste0(image_path, "lambda_R2.jpeg"), p, width=3.2, height=2, units='in')



#public charging coverage
ct_name <- paste0('charge_table_S2_R2_p=', as.character(c(1:10)/10), '.csv')
setwd("H:\\NHTS2017\\Result Data")
ct <- do.call("rbind", lapply(ct_name, read.csv))
...
p
#ggsave(paste0(image_path, "public_event_R2.jpeg"), p, width=3.2, height=2.6, units='in')
...
p
#ggsave(paste0(image_path, "public_grid_R2.jpeg"), p, width=3.2, height=2.6, units='in')
...
ct <- ldply(alpha_beta, 
       function (x) {
         setwd("H:\\NHTS2017\\Result Data")
         charge <- read.csv(paste0('charge_S2_R2_ab=', as.character(x), '.csv'))
...
ImpactOn20ChargeAlphaBeta <- function(ct) {
  p <- ggplot(ct, aes(x=alpha_beta)) +
    ...
    scale_x_continuous(limits=c(0,1), breaks=seq(0.05,0.95,0.1)) + 
    scale_y_continuous(limits=c(5,22), breaks=seq(5,22,5)) + 
    theme_bw() +
    theme(axis.title=element_text(size=9), 
          axis.text = element_text(size=8),
          panel.grid.major=element_blank(),
...
p
#ggsave(paste0(image_path, "alpha=beta_anxiety_R2.jpeg"), p, width=3.2, height=2, units='in')
...
ct <- ldply(lambda, 
            function (x) {
              setwd("H:\\NHTS2017\\Result Data")
              charge <- read.csv(paste0('charge_S2_R1_l=', as.character(x), '.csv'))
              return(nrow(subset(charge, SOC_D/100*RANGE<20))/nrow(charge)*100)
            })
colnames(ct) <- 'less_than_20'
ct$lambda <- lambda
...
p
#ggsave(paste0(image_path, "lambda_anxiety_R2.jpeg"), p, width=3.2, height=2, units='in')
...
#Result 7: compare rational drivers vs. CPT

#Scenario S2_R2
charge_S2 <- read.csv(paste0("H:\\NHTS2017\\Result Data\\charge_", "S2_R2", ".csv"))
...
charge_1 <- read.csv(paste0("H:\\NHTS2017\\Result Data\\charge_", "S2_R2_ab=0.95", ".csv"))
grid_1 <- GridPower(charge_1, "total")
grid_1$power3_ratio <- grid_1$power3/grid_1$power3
grid_1$event3_ratio <- grid_1$event3/grid_1$event3

charge_2 <- read.csv(paste0("H:\\NHTS2017\\Result Data\\charge_", "S2_R2_ab=0.05", ".csv"))
grid_2 <- GridPower(charge_2, "total")
grid_2$power3_ratio <- grid_2$power3/grid_2$power3
grid_2$event3_ratio <- grid_2$event3/grid_2$event3

charge_3 <- read.csv(paste0("H:\\NHTS2017\\Result Data\\charge_", "S2_R2_rational", ".csv"))
grid_3 <- GridPower(charge_3, "total")
grid_3$power3_ratio <- grid_3$power3/grid_3$power3
grid_3$event3_ratio <- grid_3$event3/grid_3$event3

a=max(grid_S2$power3/1000/1000)
b=max(grid_3$power3/1000/1000)
(a-b)/a

#compare rational vs CPT
ChargingProfileComparisonRational <- function(grid_S2, grid1, grid2, grid3) {
  
  grid_S2$scenario <- 'CPT'
  
  grid_sub1 <- subset(grid1, place=='total')
  grid_sub1$scenario <- paste0('CPT (', '\u03B1', ',', '\u03B2', '=0.95)')
  
  grid_sub2 <- subset(grid2, place=='total')
  grid_sub2$scenario <- paste0('CPT (', '\u03B1', ',', '\u03B2', '=0.05)')
  
  grid_sub3 <- subset(grid3, place=='total')
  grid_sub3$scenario <- 'rational drivers'
  
  grid_all <- rbind(grid_S2, grid_sub1, grid_sub2, grid_sub3)
  grid_all$scenario <- factor(grid_all$scenario, levels=c('CPT',
                                                          paste0('CPT (', '\u03B1', ',', '\u03B2', '=0.95)'),
                                                          paste0('CPT (', '\u03B1', ',', '\u03B2', '=0.05)'),
                                                          'rational drivers'))
...
p <- ChargingProfileComparisonRational(grid_S2, grid_1, grid_2, grid_3)
p
#ggsave(paste0(image_path, "load_compare_R2_rational", ".jpeg"), p, width=5, height=2.5, units='in')





#Result 8: single TOU price
...
GridPowerDelayCharging <- function(aa, place) {
  ...
  aa$charging_start[delay_condition] <- aa$starttime_next - aa$charging_duration
  #aa$charging_start[delay_condition] <- (aa$starttime_next + aa$charging_start)/2
  aa$charging_end <- aa$charging_start + aa$charging_duration
...
#Result 9: dynamic price, multiple TOU
...
charge_1 <- read.csv(paste0("H:\\NHTS2017\\Result Data\\charge_", "S2_R2_TOU1", ".csv"))
grid_1 <- GridPower(charge_1, "total")
grid_1$power3_ratio <- grid_1$power3/grid_1$power3
grid_1$event3_ratio <- grid_1$event3/grid_1$event3

charge_2 <- read.csv(paste0("H:\\NHTS2017\\Result Data\\charge_", "S2_R2_TOU2", ".csv"))
grid_2 <- GridPower(charge_2, "total")
grid_2$power3_ratio <- grid_2$power3/grid_2$power3
grid_2$event3_ratio <- grid_2$event3/grid_2$event3

charge_3 <- read.csv(paste0("H:\\NHTS2017\\Result Data\\charge_", "S2_R2_TOU3", ".csv"))
grid_3 <- GridPower(charge_3, "total")
grid_3$power3_ratio <- grid_3$power3/grid_3$power3
grid_3$event3_ratio <- grid_3$event3/grid_3$event3

charge_4 <- read.csv(paste0("H:\\NHTS2017\\Result Data\\charge_", "S2_R2_TOU4", ".csv"))
grid_4 <- GridPower(charge_4, "total")
grid_4$power3_ratio <- grid_4$power3/grid_4$power3
grid_4$event3_ratio <- grid_4$event3/grid_4$event3

a=grid_S2$power3[19]/1000/1000
b=grid_4$power3[19]/1000/1000
(a-b)/a

#compare different TOU
ChargingProfileComparisonDPMultiTOU <- function(grid_S2, grid1, grid2, grid3, grid4) {
  
  grid_S2$scenario <- 'constant rate'
  
  grid_sub1 <- subset(grid1, place=='total')
  grid_sub1$scenario <- 'TOU 1'
  
  grid_sub2 <- subset(grid2, place=='total')
  grid_sub2$scenario <- 'TOU 2'
  
  grid_sub3 <- subset(grid3, place=='total')
  grid_sub3$scenario <- 'TOU 3'
  
  grid_sub4 <- subset(grid4, place=='total')
  grid_sub4$scenario <- 'TOU 4'
  
  grid_all <- rbind(grid_S2, grid_sub1, grid_sub2, grid_sub3, grid_sub4)
  grid_all$scenario <- factor(grid_all$scenario, levels=c('constant rate','TOU 1','TOU 2','TOU 3','TOU 4'))
...
p <- ChargingProfileComparisonDPMultiTOU(grid_S2, grid_1, grid_2, grid_3, grid_4)
p
#ggsave(paste0(image_path, "load_compare_R2_multiTOU", ".jpeg"), p, width=5, height=2.5, units='in')
...
GridPowerPlot <- function(grid) {
  p <- ggplot(grid, aes(x=hour, y=power3)) + 
    geom_line() +
    geom_point() +
    labs(x='Time of day (h)', y='Charging power demand (kW)') +
    scale_x_continuous(limits=c(0, 24), breaks=seq(0,24,1)) + 
    theme_bw() +
    theme(axis.title=element_text(size=9), 
          axis.text = element_text(size=8),
...
#ggsave(paste0(image_path, image_name), p, width=5, height=2.5, units='in')

GridPowerMultiPlot <- function(grid) {
  p <- ggplot(grid, aes(x=hour, y=power3/1000/1000, colour=place)) + 
    geom_line() +
    #geom_line(aes(linetype=place)) +
    #geom_point() +
    geom_point(aes(shape=place)) +
    labs(x='Time of day (h)', y='Electricity demand (GW)') +
    scale_x_continuous(limits=c(0, 24), breaks=seq(0,24,1)) + 
    theme_bw() +
    theme(axis.title=element_text(size=9), 
          axis.text = element_text(size=8),
          ...
          panel.grid.minor=element_blank(),
          #legend
          legend.position=c(0.3,0.9), #c(0.3,0.9) OR c(0.15,0.5)
          legend.margin=margin(0,0,0,0),
          legend.direction="horizontal",
          legend.text=element_text(size=8),
          legend.title=element_blank()) 
...
p <- GridPowerMultiPlot(grid)
p
#ggsave(paste0(image_path, "grid_", scenario, ".jpeg"), p, width=5, height=2.5, units='in')

GridPowerStackedBar <- function(grid) {
  grid <- subset(grid, place!='total')
  grid$place <- factor(grid$place, levels=c('public','work','home'))
  p <- ggplot(grid, aes(x=hour, y=event3/1000/1000, fill=place)) + #power3/1000/1000
    geom_bar(stat='identity') +
    labs(x='Time of day (h)', y='Charges (million)') + #Electricity demand (GW)
    scale_x_continuous(limits=c(0,24), breaks=seq(0,24,1)) + 
    theme_bw() +
    theme(axis.title=element_text(size=10), 
          axis.text = element_text(size=9),
...
p <- GridPowerStackedBar(grid)
p
#ggsave(paste0(image_path, "grid_stackbar_", scenario, ".jpeg"), p, width=5, height=2.5, units='in')
#ggsave(paste0(image_path, "event_stackbar_", scenario, ".jpeg"), p, width=5, height=2.5, units='in')

GridPowerStackedLine <- function(grid) {
  grid <- subset(grid, place!='total')
  grid$place <- factor(grid$place, levels=c('home','work','public'))
  p <- ggplot(grid, aes(x=hour, y=event3_ratio*100, color=place)) +
    geom_line() +
    geom_point(aes(shape=place)) +
    labs(x='Time of day (h)', y='Proportion of total charges (%)') +
    scale_x_continuous(limits=c(0,24), breaks=seq(0,24,1)) + 
    theme_bw() +
    theme(axis.title=element_text(size=10), 
          axis.text = element_text(size=9),
...
p <- GridPowerStackedLine(grid)
p
#ggsave(paste0(image_path, "grid_prop_", scenario, ".jpeg"), p, width=5, height=2.5, units='in')
#ggsave(paste0(image_path, "event_prop_", scenario, ".jpeg"), p, width=5, height=2.5, units='in')

ImpactOnFeasibility <- function(ct) {
  ct <- subset(ct, place=='total')
  p <- ggplot(ct, aes(x=coverage, y=feasibility)) +
    geom_line() + 
    geom_point() +
    labs(x='Parameter', y='Feasibility') +
    #scale_x_continuous(limits=c(0,1), breaks=c(0,0.1,0.3,0.5,0.7,0.88,1)) + 
    theme_bw() +
    theme(axis.title=element_text(size=10), 
          axis.text = element_text(size=9),
          panel.grid.major=element_blank(),
...
ImpactOnFeasibility(ct)

ChargingProfileComparisonDP <- function(grid, grid_S2) {
  
  grid_sub <- subset(grid, place=='total')
  grid_sub$scenario <- 'dynamic rate'
  grid_S2$scenario <- 'constant rate'
  grid_all <- rbind(grid_S2, grid_sub)
...
p <- ChargingProfileComparisonDP(grid, grid_S2)
p
#ggsave(paste0(image_path, "load_compare_", scenario, ".jpeg"), p, width=5, height=2.5, units='in')
...
#not useful code
if (0) {
hist(aa$SOC_D[aa$DESTINATION=='home'])
summary(aa$SOC_D[aa$DESTINATION=='home'])
hist(aa$SOC_D[aa$DESTINATION=='work'])
summary(aa$SOC_D[aa$DESTINATION=='work'])
hist(aa$SOC_D[aa$DESTINATION=='public'],
     main='public charger=yes, decision=yes', xlab='arrival SOC (%)', breaks=seq(0,100,10))
summary(aa$SOC_D[aa$DESTINATION=='public'])
sd(aa$SOC_D[aa$DESTINATION=='public'])
#check dwell time at charge
hist(aa$DWELTIME_new)
hist(aa$DWELTIME_new[aa$DESTINATION=='home'])
hist(aa$DWELTIME_new[aa$DESTINATION=='work'])
hist(aa$DWELTIME_new[aa$DESTINATION=='public'])


#check arrival SOC where charger is available, but decide not to charge
bb=subset(df, Charger==1 & Decision==0)
hist(bb$SOC_D, main='charger=yes, decision=no', xlab = 'arrival SOC (%)')
summary(bb$SOC_D)
hist(bb$SOC_D[bb$DESTINATION=='home'])
hist(bb$SOC_D[bb$DESTINATION=='work'])
hist(bb$SOC_D[bb$DESTINATION=='public'])

#check arrival SOC distribution
cc=subset(df, df$Charger==1)
hist(cc$SOC_D, main='histogram for arrival SOC', xlab = 'arrival SOC (%)')
summary(cc$SOC_D)

ImpactOnSOC <- function(ct) {
  ct <- subset(ct, place=='total')
  p <- ggplot(ct, aes(x=coverage, y=SOC_mean)) +
    geom_line() + 
    geom_point() +
    labs(x='Parameter', y='Average SOC (%)') +
    #scale_x_continuous(limits=c(0,1), breaks=c(0,0.1,0.3,0.5,0.7,0.88,1)) + 
    theme_bw() +
    theme(axis.title=element_text(size=10), 
          axis.text = element_text(size=9),
